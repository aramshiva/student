...
export function calculateCourseGradePercentageFromCategories(
  ...
  for (const cat of gradeCategories) {
    ...
    weightedSum +=
      (pts.pointsEarned / pts.pointsPossible) * cat.weightPercentage;
...
export function getAssignmentPointTotals<T extends AssignmentBase>(
...
export function calculateCourseGradePercentageFromTotals<
  T extends AssignmentBase,
>(assignments: Calculable<T>[]) {
  const { pointsEarned, pointsPossible } =
    getAssignmentPointTotals(assignments);
...
function flowAssignmentFromTotals<T extends AssignmentBase>(
  ...
  return {
    assignment: {
      ...assignment,
      gradePercentageChange: afterGrade - priorGrade,
    },
    totalPointsEarned,
    totalPointsPossible,
  };
...
function flowAssignmentFromCategories<T extends AssignmentBase>(
  ...
  const existing = pointsByCategory[assignment.category] || {
    pointsEarned: 0,
    pointsPossible: 0,
  };
  pointsByCategory[assignment.category] = {
    ...
    pointsPossible:
      ...
      (assignment.extraCredit ? 0 : assignment.pointsPossible),
  };
...
export function calculateAssignmentGPCsFromCategories<T extends AssignmentBase>(
  assignments: T[],
  gradeCategories: Category[],
) {
  ...
  for (let i = assignments.length - 1; i >= 0; i--) {
    ...
    const flowed = flowAssignmentFromCategories(
      calculable,
      pointsByCategory,
      gradeCategories,
    );
...
export function calculateAssignmentGPCsFromTotals<T extends AssignmentBase>(
  assignments: T[],
) {
  const out: (T | Flowed<T>)[] = new Array(assignments.length);
  ...
  for (let i = assignments.length - 1; i >= 0; i--) {
    ...
    const flowed = flowAssignmentFromTotals(
      calculable,
      totalPointsEarned,
      totalPointsPossible,
    );
...
export function calculateAssignmentGPCs<T extends AssignmentBase>(
  assignments: T[],
  gradeCategories?: Category[],
) {
...
export function getPointsByCategory<T extends AssignmentBase>(
  assignments: Calculable<T>[],
) {
  const pointsByCategory: PointsByCategory = {};
  for (const a of assignments) {
    const existing = pointsByCategory[a.category] || {
      pointsEarned: 0,
      pointsPossible: 0,
    };
    pointsByCategory[a.category] = {
      ...
      pointsPossible:
        existing.pointsPossible + (a.extraCredit ? 0 : a.pointsPossible),
    };
  }
...
export function gradesMatch(rawGrade: number, expectedGrade: number) {
  const leastPrecision = Math.min(
    countDecimalPlaces(rawGrade),
    countDecimalPlaces(expectedGrade),
  );
  const roundedMatches =
    roundToPrecision(rawGrade, leastPrecision) ===
    roundToPrecision(expectedGrade, leastPrecision);
  const flooredMatches =
    floorToPrecision(rawGrade, leastPrecision) ===
    floorToPrecision(expectedGrade, leastPrecision);
...
export function getSynergyCourseAssignmentCategories(course: SynergyCourse) {
  ...
  if (
    typeof gradeCalcSummary === "string" ||
    !gradeCalcSummary?.AssignmentGradeCalc
  )
    return undefined;

  const categories: Category[] = (
    gradeCalcSummary.AssignmentGradeCalc as SynergyAssignmentGradeCalc[]
  ).map((category) => ({
    name: category._Type,
...
export function getCalculableAssignments<T extends AssignmentBase>(
  assignments: T[],
) {
  return assignments
    .map((assignment) => {
...
export function parseSynergyAssignment(
  synergyAssignment: SynergyAssignment,
): RealAssignment {
  const {
    _Date,
    _Measure,
  ...
  const assignment: RealAssignment = {
    ...
    date: new Date(_Date),
    comments:
      notesFormatted && notesFormatted.length > 0 ? notesFormatted : undefined,
  };
