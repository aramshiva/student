// thank you https://github.com/Jshap06/GradeMelon2/blob/APIVersion/lib/districts.ts
// for compiling this :)
...
function extractZip(address: string): string | undefined {
  const m = address.match(/(\d{5})(?:-\d{4})?$/);
  return m ? m[1] : undefined;
}

interface EnrichedDistrict extends DistrictEntry {
  _normName: string;
  zipcode?: string;
}

const enriched: EnrichedDistrict[] = districts.map((d) => ({
  ...d,
  zipcode: d.zipcode || extractZip(d.address) || undefined,
  _normName: d.name.toLowerCase(),
}));

export interface LocalDistrictSearchOptions {
  zip?: string;
  name?: string;
  radius?: number;
  limit?: number;
...
export interface LocalDistrictSearchResult {
  input: LocalDistrictSearchOptions;
  fuzzyZip: boolean;
  searchedZips: string[];
  results: DistrictEntry[];
}

export function searchDistrictsLocal(opts: LocalDistrictSearchOptions): LocalDistrictSearchResult {
  const { zip, name, radius = 0, limit = 200 } = opts;
  const normName = name?.trim().toLowerCase();
  let searchedZips: string[] = [];
  let candidates: EnrichedDistrict[] = enriched;
  let fuzzyZip = false;

  if (zip && /^(\d{5})$/.test(zip)) {
    const base = parseInt(zip, 10);
    const r = Math.min(Math.max(radius, 0), 50);
    const zipSet = new Set<string>();
    zipSet.add(zip);
    for (let off = 1; off <= r; off++) {
      const low = (base - off).toString().padStart(5, "0");
      const high = (base + off).toString().padStart(5, "0");
      if (parseInt(low, 10) >= 0) zipSet.add(low);
      if (parseInt(high, 10) <= 99999) zipSet.add(high);
    }
    searchedZips = Array.from(zipSet.values());
    fuzzyZip = searchedZips.length > 1;
    candidates = enriched.filter((d) => d.zipcode && zipSet.has(d.zipcode));

    if (!candidates.length) {
      const allZipEntries = enriched.filter((d) => d.zipcode).map((d) => d.zipcode!) as string[];
      const numericTargets = allZipEntries.map((z) => parseInt(z, 10)).filter((n) => !isNaN(n));
      const windowMin = base - 150;
      const windowMax = base + 150;
      const closeSet = new Set(
        numericTargets
          .filter((n) => n >= windowMin && n <= windowMax)
          .sort((a, b) => Math.abs(a - base) - Math.abs(b - base))
          .slice(0, 120)
          .map((n) => n.toString().padStart(5, "0")),
      );
      const fallback = enriched.filter((d) => d.zipcode && closeSet.has(d.zipcode));
      if (fallback.length) {
        candidates = fallback;
        searchedZips = Array.from(new Set([...searchedZips, ...Array.from(closeSet.values())]));
        fuzzyZip = true;
      }
    }
  }
  ...
  if (normName) {
    candidates = candidates.filter((d) => d._normName.includes(normName));
  }

  const seen = new Set<string>();
  const unique: DistrictEntry[] = [];
  for (const c of candidates) {
    const key = c.parentVueUrl.toLowerCase();
    if (!seen.has(key)) {
      seen.add(key);
      unique.push({ address: c.address, name: c.name, parentVueUrl: c.parentVueUrl, zipcode: c.zipcode });
    }
